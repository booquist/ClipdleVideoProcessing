...
const uploadGcsRouter = express.router();
...
async function uploadVideoToGCS(file: any): Promise<{ filename: string, aspectRatio: string }> {
    ...
    try {
        ...
        const metadata: any = await new Promise((resolve, reject) => {
            ffmpeg.ffprobe(tmpFilePath, (err: any, metadata: unknown) => {
                if (err) {
                    return reject(err);
                }
                resolve(metadata);
...
async function uploadProfilePictureToGCS(file: any, username: string): Promise<string> {
...
async function deleteProfilePictureFromGCS(pictureUrl: string): Promise<void> {
...
async function createAndUploadThumbnail(videoFilePath: string, videoId: string): Promise<string> {
